...
def generate_data(seed):
	np.random.seed(seed) # fix the random generated number pattern
	data_size_1 = 400
	x1_1 = np.random.normal(loc = 5.0, scale=1.0, size=data_size_1)
	x2_1 = np.random.normal(loc = 4.0, scale=1.0, size=data_size_1)
	'''	plt.hist(x1_1,50,normed=1,facecolor='g',alpha=0.75)
	plt.xlabel('Number')
	plt.ylabel('Generated Number')
	plt.title('Histogram of x1_1')
	plt.text(3,0.5,r'$\mu=1$')
	plt.axis([1,9,0,0.8])
	plt.grid(True)
	plt.show() '''
	pdb.set_trace()
	...
	y_2  = [1 for _ in range(data_size_2)] # _ is used as a variable
	x1 = np.concatenate((x1_1,x1_2),axis = 0)
	x2 = np.concatenate((x2_1,x2_2),axis = 0)
	
	x = np.hstack((x1.reshape(-1,1),x2.reshape(-1,1)))
	y = np.concatenate((y_1,y_2),axis = 0) # p y.shape can check its size; y.size; len(y_1)
	data_size_all = data_size_1 + data_size_2
	shuffled_index = np.random.permutation(data_size_all) # permutation return a new array while shuffled disturb the original array
	x = x[shuffled_index]
	y = y[shuffled_index]
	return x,y
